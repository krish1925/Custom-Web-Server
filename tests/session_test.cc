...
protected:
    ...
    std::shared_ptr<MockResponseHandler> mock_handler; // Mock response handler for testing
...
{
    Session session(io_service_, *config_manager_, mock_handler);
...
public:
    TestableSession(boost::asio::io_service &io_service,
        ...
        : Session(io_service, config, handler) {}
...
// TODO: correctly implement with a mock request handler
// // HandleReadComplete test
// TEST_F(SessionTest, HandleReadComplete)
// {
//     TestableSession session(io_service_, *config_manager_, mock_handler);
